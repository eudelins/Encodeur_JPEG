...
struct Pixel_YCbCr *conversion(Pixel_RGB *pixelRGB)
{
  struct Pixel_YCbCr *pixelYCbCr = malloc(sizeof(Pixel_YCbCr));
...
int main(void)
{
  ...
  struct Pixel_RGB *pixel = &pixelRGB;
  printf("La valeur du pixel RGB est : %u, %u, %u \n", pixel->R, pixel->G, pixel->B);

  struct Pixel_YCbCr *converti = conversion(pixel);
